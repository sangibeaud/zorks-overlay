...
EBUILDS=($(git diff --name-only --diff-filter=d "HEAD^" "HEAD" | grep '\.ebuild$')) || true
...
if [ ${#EBUILDS[@]} -eq 0 ]; then
  ...
  PACKAGES=($(git diff --name-only --diff-filter=d HEAD^ HEAD ':!tests' | awk -F/ '{if (NF>=3) print $1"/"$2;}' | sort -u)) || true
